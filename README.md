...
language and `forall` were a reserved identifier. In respect of changes to
modules since the earlier report, its approach is to make only changes that are
strictly necessary to be consistent with those language extensions and by
reference to the boot packages of GHC 9.10.3 as published on Hackage.
