-- This generates the prelude index to the report by hyper-linking all of the
-- names in a hand-written document. This is probably too obscure to bother
-- explaining. Hardwires for the Haskell report at the moment.
...
expandAll1 _ _ [] = []
expandAll1 r table (l : ls)
  | l == "#table" = expandAll1 r True ls
  | l == "#endtable" = expandAll1 r False ls
  | table =   ("<tr><td><tt>" ++ nbspaces (expandRefs r l) ++ "</tt></td></tr>")
            : rest
  | otherwise = expandRefs r l : rest
 where
  rest = expandAll1 r table ls
...
expandTycon r txt (t, rest) =
  let n = mangleTycon t
      f = lookup n r
  in  anchor t f n txt ++ expandRefs r rest

expandInstance :: Refs -> String -> (String, String) -> String
expandInstance r txt (c, '#' : rest ) =
  let (t, rest') = parseRef rest
      n = mangleInstance c t
      f = lookup n r
  in  anchor c f n txt ++ expandRefs r rest'
expandInstance _ _ (_, l) = error ("bad instance " ++ l ++ "\n")

expandSect :: Refs -> String -> (String, String) -> String
expandSect r txt (s, rest) =
  let n = mangleSect s
      f = lookup n r
  in  "(see " ++ anchor s f n txt ++ ")" ++ expandRefs r rest
...
trySig r txt v rest _n =
  let c = parseClass rest
      n = mangleTycon c
      f = lookup n r
  in  anchor v f n txt ++ expandRefs r rest

anchor :: String -> Maybe String -> String -> String -> String
anchor str mfile tag txt = case mfile of
  Just f -> "<a href=\"" ++ f ++ ".html#" ++ tag ++ "\">" ++ t ++ "</a>"
...
mangleName = concatMap $ \c -> case c of
  '(' -> "$P"
  ')' -> "$C"
  '-' -> "$D"
  '[' -> "$B"
...
readRefFile f = catchIOError
  ...
  ( \e -> do
      putStrLn ("Can't read ref file: " ++ f)
      print e
      return []
  )
